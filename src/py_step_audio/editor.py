...
class WaveformTrimUI(tk.Toplevel):
    def __init__(self, master, audio_segment, on_trim_callback):
        ...
        self.start_slider = tk.Scale(self, from_=0, to=len(self.samples), orient='horizontal',
                                     label='Start', command=self.update_start)
        self.start_slider.pack(fill='x')

        self.end_slider = tk.Scale(self, from_=0, to=len(self.samples), orient='horizontal',
                                   label='End', command=self.update_end)
        ...
        tk.Button(btn_frame, text="Apply Trim", command=self.apply_trim).pack(side='left')
